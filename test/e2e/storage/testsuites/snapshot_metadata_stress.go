...
func (s *snapshotMetadataStressTestSuite) SkipUnsupportedTests(driver storageframework.TestDriver, pattern storageframework.TestPattern) string {
	...
	if driverInfo.SnapshotMetadataStressTestOptions.NumPods <= 0 {
		framework.Failf("NumPods in snapshot metadata stress test options must be a positive integer, received: %d", driverInfo.SnapshotMetadataStressTestOptions.NumPods)
	}
	if driverInfo.SnapshotMetadataStressTestOptions.NumSnapshotPairs <= 0 {
		framework.Failf("NumSnapshotPairs in snapshot metadata stress test options must be a positive integer, received: %d", driverInfo.SnapshotMetadataStressTestOptions.NumSnapshotPairs)
	}
...
func (s *snapshotMetadataStressTestSuite) DefineTests(driver storageframework.TestDriver, pattern storageframework.TestPattern) {
	...
	f := framework.NewDefaultFramework("snapshot-metadata-stress")
	...
	cleanup := func(ctx context.Context) {
		...
		for _, pvc := range stressTest.restoredPVCs {
			go func(pvc *v1.PersistentVolumeClaim) {
				...
				err := cs.CoreV1().PersistentVolumeClaims(pvc.Namespace).Delete(ctx, pvc.Name, metav1.DeleteOptions{})
				mu.Lock()
				defer mu.Unlock()
				errs = append(errs, err)
			}(pvc)
